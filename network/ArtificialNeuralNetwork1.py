...
class ArtificialNeuralNetwork:
	...
	def train(self, inputs, targets, iterations):
		for iteration in range(iterations):
			...
			print("hypothesis: " + str(hypothesis.collect()))
	...
	def back_propagate(self, hypothesis, target):
		...
		for layer in range(self.num_layers()-1, -1, -1):
			...
			if (is_adjacent_to_output_layer(layer, self.num_layers())):
				...
				print("weights connecting upper layer to this layer: "
					+ str(weights))
				# Optimisation to be made here if this is the second-highest layer - doesn't need 
				# to re-compute activations. 
				upper_layer_activations = self.activations[layer+1].map(lambda x: 
					np.mean(x, axis = 0)).persist()

				print("summarised upper layer activations: " 
					+ str(upper_layer_activations.collect()))
				if (layer == self.num_layers()-2):
					h_error = out_error
				else:
					h_error = hidden_error
				print("error from connected upper layer: " + str(
					h_error.collect()))
				hidden_error_activations = h_error.zip(upper_layer_activations).persist()
				...
				print("current-layer error (pre-agg): " + str(hidden_error.collect()))
				temp = hidden_error
				layer_activations = self.activations[layer].map(lambda x: 
					np.mean(x, axis = 0)).persist()
				#seqOp = (lambda x, y: (x[0] + y, x[1] + 1))
				#combOp = (lambda x, y: (x[0] + y[0], x[1] + y[1]))
				errors_activations = hidden_error.zip(layer_activations).persist()
				...
				print("weight_gradients[layer] (pre-agg): " + str(weight_gradients[layer].collect()))
				seqOp = (lambda x, y: (x[0] + y, x[1] + 1))
				combOp = (lambda x, y: (x[0] + y[0], x[1] + y[1]))
				...
				print("current-layer error (post-agg): " + str(hidden_error))
				seqOp = (lambda x, y: (x[0] + y, x[1] + 1))
				combOp = (lambda x, y: (x[0] + y[0], x[1] + y[1]))
				weight_gradients[layer] = weight_gradients[layer].aggregate((np.zeros((weight_gradients[layer].collect()[0].shape[0],weight_gradients[layer].collect()[0].shape[1])),0), seqOp, combOp)
				weight_gradients[layer] = weight_gradients[layer][0]/float(weight_gradients[layer][1])
				print("weight_gradients[layer] (post-agg): " + str(weight_gradients[layer]))

			self.layers.get_layer(layer).get_weights()[:self.layers.get_layer(layer).get_weights().shape[0]-1] -= (
				self.layers.get_layer(layer).get_learning_rate() * ((weight_gradients[layer][weight_gradients[layer].shape[0]-1] if 
					layer != (self.num_layers()-1) else weight_gradients[layer][weight_gradients[layer].shape[0]-1]) + 
						self.layers.get_layer(layer).get_weight_penalty() * \
						self.layers.get_layer(layer).get_weights()[:self.layers.get_layer(layer).get_weights().shape[0]-1]))
			if (hidden_error != None):
				self.layers.get_layer(layer).get_weights()[self.layers.get_layer(layer).get_weights().shape[0]-1] -=(hidden_error.flatten())
			self.layers.get_layer(layer).get_weights()[self.layers.get_layer(layer).get_weights().shape[0]-1] -= (
