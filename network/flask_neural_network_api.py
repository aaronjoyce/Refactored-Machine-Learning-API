...
@app.route('/genomex/api/v1.0/network', methods = ['POST'])
#@auth.login_required
def create_network():
	networks.append({str(most_recent_network_id+1): NeuralNetworkAPI(sc)})
	most_recent_network_id += 1
	return jsonify({'Status: ' : 'Network successfully created'})
...
if __name__ == '__main__':
	#sc = SparkContext(appName = "DistributedNetworkAPI.py")
	app.run(debug = True)
